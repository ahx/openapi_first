...
RSpec.describe OpenapiFirst::Definition::RequestBody do
  describe '#schema_for' do
    ...
    it 'returns a schema for an exact match' do
      request_body = described_class.new({ 'content' => { 'application/json' => media_type } })
    ...
    it 'matches type/*' do
      request_body = described_class.new({ 'content' => { 'text/*' => media_type } })
      media_type = request_body.schema_for('text/plain')
      expect(media_type).to be_truthy
    end

    it 'matches */*' do
      request_body = described_class.new({ 'content' => { '*/*' => media_type } })
      media_type = request_body.schema_for('application/json')
      expect(media_type).to be_truthy
    end
    ...
    it 'returns nil for a miss match' do
      request_body = described_class.new({ 'content' => { 'application/xml' => media_type } })
      media_type = request_body.schema_for('application/json')
      expect(media_type).to be_nil
    end
  end

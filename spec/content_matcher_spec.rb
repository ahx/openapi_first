...
RSpec.describe OpenapiFirst::ContentMatcher do
  describe '#call' do
    ...
    it 'returns empty list if no match' do
      expect(matcher.call('image/*')).to be_empty
    end

    it 'finds an exact match with parameter' do
      exact = 'application/json; profile=custom'
      expect(matcher.call(exact).map(&:content_type)).to eq([exact])
    end

    it 'finds a match while ignoring parameter' do
      expect(matcher.call('application/xml; Charset=Utf8').map(&:content_type)).to eq(['application/xml'])
    end

    it 'finds text/* wildcard matcher' do
      expect(matcher.call('text/markdown; Charset=Utf8').map(&:content_type)).to eq(['text/*'])
    end
    ...
    it 'finds */* wildcard matcher' do
      ...
      expect(matcher.call('some/foobar').map(&:content_type)).to eq(['*/*'])
      expect(matcher.call('some/foobar; Chartset=utf8').map(&:content_type)).to eq(['*/*'])
    end
  end
end
